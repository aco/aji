...
@objc public final class BLTNItemManager: NSObject {
	...
	internal var bulletinController: BulletinViewController!
	...
	*/
	...
	@objc public var backgroundColor = Style.Colors.mainBackground
...
extension BLTNItemManager {
	...
	*/
	...
	public func dismissBulletin(animated: Bool = true) {
		...
		DispatchQueue.main.async {
			self.currentItem.tearDown()
			self.currentItem.manager = nil
			
			self.bulletinController.dismiss(animated: animated) {
	...
	*/
	...
	@nonobjc func completeDismissal() {
		...
		guard bulletinController != nil else {
			return
		}
...
extension BLTNItemManager {
	...
	fileprivate func refreshCurrentItemInterface(elementsChanged: Bool = true) {
		...
		let newArrangedSubviews = currentItem.makeArrangedSubviews(buttonAxis: bulletinController.buttonAxis)
