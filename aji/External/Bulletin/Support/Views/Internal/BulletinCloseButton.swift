...
class BulletinCloseButton: UIControl {
	...
	private let closeGlyph = Factory.generateIconLabel(icon: "xmark", color: Style.Colors.secondaryText, size: 10)
...
//		closeGlyph.textAlignment = .center
//		closeGlyph.contentMode = .scaleAspectFit
		...
		backgroundContainer.insertSubview(blurView, at: 0)
	}
	...
	private func configureConstraints() {
		...
		NSLayoutConstraint.activate([
			blurView.heightAnchor.constraint(equalTo: backgroundContainer.heightAnchor),
			blurView.widthAnchor.constraint(equalTo: backgroundContainer.widthAnchor),
		])
	}
